...
const CarauselCard = ({ items = [] }) => {
  ...
  const handleTransitionEnd = () => {
    ...
    if (currentIndex === extendedItems.length - 1) {
      ...
      containerRef.current.style.transition = "none";
      containerRef.current.style.transform = `translateX(-${(extendedItems.length - 2) * 100}%)`;
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          containerRef.current.style.transition = "transform 0.5s ease-in-out";
        });
      ...
      <div className="relative overflow-hidden rounded-2xl shadow-lg bg-white">
          ...
          {items.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                index + 1 === currentIndex ? "bg-white scale-125" : "bg-white/50 hover:bg-white/75"
              }`}
              disabled={isAnimating}
            />
          ))}
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20">
          <div
            ...
            style={{
              width: `${((currentIndex - 1 + items.length) % items.length + 1) * (100 / items.length)}%`,
            }}
          />
        </div>
      </div>
